...
export function MarqueeEditor({ sectionId, content }: Props) {
  ...
  function handleSave() {
    startTransition(async () => {
      await saveSection(sectionId, { items: items.filter(Boolean) });
      setSaved(true);
    ...
    <div className="space-y-6">
      <div className="rounded-lg border border-gray-200 bg-white p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-gray-800">Marquee Items</h2>
          <button
            type="button"
        ...
        <p className="text-xs text-gray-400">These scroll across the strip below the hero.</p>
        {items.map((item, i) => (
